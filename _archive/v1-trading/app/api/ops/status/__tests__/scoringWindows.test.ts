...
describe("computeScoringWindows", () => {
  test("computes created and scored windows separately", () => {
    ...
    expect(result.createdLast6Hours.archived).toBe(0);
  ...
  test("counts archived skips in created window", () => {
    const now = new Date("2026-02-07T12:00:00Z");
    const signals = [
      {
        id: "s1",
        status: "ARCHIVED",
        createdAt: "2026-02-07T11:50:00Z",
        scoredAt: "2026-02-07T11:55:00Z",
      },
      {
        id: "s2",
        status: "PENDING",
        createdAt: "2026-02-07T11:40:00Z",
      },
    ];
    ...
    expect(result.createdLast6Hours.total).toBe(2);
    expect(result.createdLast6Hours.pending).toBe(1);
    expect(result.createdLast6Hours.archived).toBe(1);
  });
});
